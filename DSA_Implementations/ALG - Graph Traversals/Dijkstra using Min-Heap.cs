...
public class DijkstrausingMinHeap
{
    ...
    public DijkstrausingMinHeap(List<string> vertices, enGraphDirectionType GraphDirectionType)
    {
    ...
    {
        // This implementation will use: Adjacency List with Min-Heap: O((V+E) log⁡ V) for better performance.
        /*
         The priority queue is used in Dijkstra's algorithm to efficiently retrieve the vertex with the smallest distance at each step. 
         It helps maintain an ordering of vertices based on their shortest distance (priority).
         */
        ...
        /*
           ...
           SortedSet<(int distance, int vertexIndex)>:
           The SortedSet stores tuples of type (int distance, int vertexIndex):
           distance: The current shortest known distance from the starting vertex to this vertex.
           vertexIndex: The index of the vertex in the adjacency matrix.
           SortedSet automatically keeps its elements sorted based on a comparison logic provided by the Comparer.

            The Comparer<(int distance, int vertexIndex)> defines how elements in the SortedSet are ordered. 
            The Create method is used to define a custom comparison function.

           Comparison Rule:
              If the distance values of two elements are equal, compare their vertexIndex to break the tie.
                x.vertexIndex.CompareTo(y.vertexIndex) ensures deterministic ordering when distances are equal.
               Otherwise, compare based on distance.
               x.distance.CompareTo(y.distance) ensures the smallest distance comes first.
